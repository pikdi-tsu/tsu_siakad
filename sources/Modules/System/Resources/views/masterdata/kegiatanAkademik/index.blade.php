                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body">
                            ...
                            <div id="form-container" style="display: none;" class="mb-4 p-3 border rounded bg-light">
                                <h5 class="text-primary mb-3" id="form-title"><i class="fas fa-edit"></i> Input Kegiatan Akademik</h5>
                                    ...
                                    <div class="row">
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="kode_kegiatan" id="kode_kegiatan" class="form-control" placeholder="01" required>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <div class="form-group">
                                                <label>Nama Kegiatan Akademik <span class="text-danger">*</span></label>
                                                <input type="text" name="nama_kegiatan" id="nama_kegiatan" class="form-control" placeholder="Contoh: KKN / Wisuda" required>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    <input type="text" name="warna_background" id="warna_background" class="form-control" placeholder="#RRGGBB" value="#ffffff">
                                                    <div class="input-group-append">
                                                        <span class="input-group-text p-1">
                                                            <input type="color" id="color_picker" value="#ffffff" style="border: none; background: none; height: 25px; width: 30px; cursor: pointer;">
                                                        </span>
                                                    </div>
                                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#table-kegiatan').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('kegiatan_akademik.index') }}",
                columns: [
            ...
            $('#form-kegiatan').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kegiatan_akademik.store') }}",
                    data: formData,
            ...
            $('body').on('click', '.btn_edit', function() {
                var id = $(this).data('id');
                $.get("{{ route('kegiatan_akademik.index') }}" + '/edit/' + id, function(res) {
                    if (res.status == 'success') {
                        $('#id').val(res.data.id);
            ...
            $('body').on('click', '.btn_hapus', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('kegiatan_akademik.index') }}" + '/delete/' + id,
                            success: function(res) {
