    ...
    <script>
        $(document).ready(function() {
            ...
            let table = $('#table-gedung').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('perguruan_tinggi.gedung.index') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        orderable: false,
                        searchable: false
            ...
            $('#form-gedung').submit(function(e) {
                e.preventDefault();
                $.post("{{ route('perguruan_tinggi.gedung.store') }}", $(this).serialize(), function(res) {
            ...
            $('body').on('click', '.btn_edit', function() {
                let id = $(this).data('id');
                $.get("{{ route('perguruan_tinggi.gedung.edit', ':id') }}".replace(':id', id), function(
                    res) {
                    if (res.status === 'success') {
                        $('#id').val(res.data.id);
            ...
            $('body').on('click', '.btn_hapus', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'DELETE',
                            url: "{{ route('perguruan_tinggi.gedung.delete', ':id') }}"
                                .replace(':id', id),
